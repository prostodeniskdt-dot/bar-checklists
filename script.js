...
const CHECKLISTS = {
  ...
  closing: {
    ...
    // ← вернул твой исходный подробный текст + нужные правки
    items: [
  ...
  prep: {
    ...
    // ← вернул твой исходник детально + разделил пункт про льдогенератор на два
    items: [
      ...
      "бланк замывки ледогенератора заполнен (фото)",
...
// ===== Ниже — вся логика (авторизация, таймер, валидация, PDF/Share) без изменений =====
...
// ------------------------- ФОТО обязательно только где нужно -------------------------
const SPECIAL_REQUIRED_SUBSTRINGS = [
  "скрин поста в группе",     // для стоп/старт-листа
  "стоп лист ретей",
  "фото салфеток в шкафу"     // для минажей
];
